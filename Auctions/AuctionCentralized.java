...
public class AuctionCentralized implements AuctionBehavior {
	...
	public Long askPrice(Task task) {
        ...
        centralized.change_cycle(100);

		if (vehicle.capacity() < task.weight)
			return null;

		double future_reward = valueFunction[currentCity.id];
		double actual_reward = valueFunction[task.deliveryCity.id];
		...
		TaskSet before = agent.getTasks();
		if (!before.isEmpty()) {
			Variables vars_before = centralized.StochasticLocalSearch(agent.vehicles(), before);
			double cost_before = centralized.CalculateCost(agent.vehicles(), before, vars_before);
			before.add(task);
			Variables vars_after = centralized.StochasticLocalSearch(agent.vehicles(), before);
			double cost_after = centralized.CalculateCost(agent.vehicles(), before, vars_after);
			marginalCost = (cost_after - cost_before)*vehicle.costPerKm();
		}
